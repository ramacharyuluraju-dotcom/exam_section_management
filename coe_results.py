...
if not selected_cycle_id:
 st.error("⚠ CRITICAL ERROR: No Exam Cycle Selected. Please select a cycle in the Sidebar.")
 st.stop()
...
if show_cie:
 ...
 if ignored_count > 0: st.warning(f"⚠ Blocked {ignored_count} records (Not registered).")
...
if show_decoder:
 ...
 if not key_file or not bundle_files: st.warning("⚠ Please upload both the Master Key and at least one Evaluator Bundle.")
 else:
...
if show_see:
 ...
 if ignored_count > 0: st.warning(f"⚠ Blocked {ignored_count} unregistered records.")
...
if show_grading:
 ...
 if st.button("⚙ Execute Master Grading Algorithm", type="primary"):
...
if show_mod:
 with t5:
 st.subheader("⚖ Moderation & Third Valuation Engine")
 ...
 mod_tabs = st.tabs(["📂 Bulk Moderation Upload", "📥 Export 3rd Valuation List", "👤 Manual Grace Marks", "⚖ 3rd Valuation Upload"])
 ...
 c1.metric("⬆ Upgraded (Mod > Orig)", stats["upgraded"])
 ...
 with st.expander(f"⚠ {cc} - {title} (Current Grade: {c_grade})"):
 ...
 else: st.warning(f"⚠ Audited, but student still failing. New Grade: **{grd}**.")
 ...
 if f_third and st.button("⚖ Apply VTU 3rd Valuation Rules", type="primary"):
...
if show_ledgers:
 ...
 if st.button("🖨 Generate Master Ledger & PDFs"):
