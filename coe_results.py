...
def apply_grading_rules(cie_raw, see_raw, status, credits, max_cie=50, max_see=50, exam_conducted_for=100, is_pg=False):
    ...
    if status in ['MALPRACTICE', 'MP']:
        return 0, 0, 'MP', 0, False, 'MALPRACTICE'
    ...
    if credits == 0:
        if is_pass:
            return see_scaled, total, 'PP', 0, True, status
        else:
            return see_scaled, total, 'NP', 0, False, status
...
def generate_marks_card_pdf(buffer, usn, name, results_list, sgpa, has_pending=False):
    ...
    for i, row in enumerate(results_list):
        if row['grade'] in ['F', 'NP', 'AB', 'WH', 'MP']:
            col = colors.red
        elif row['grade'] in ['PND', 'PENDING']:
            col = colors.darkorange
        else:
            col = colors.green
        ...
        with col_c2:
            with st.form("manual_cie"):
                ...
                if st.form_submit_button("Save CIE Mark"):
                    ...
                    if not regs:
                        st.error(f"❌ Student {m_usn} is NOT registered for {m_cc}.")
                    else:
                ...
                with st.spinner("Decrypting Dummy IDs and extracting marks..."):
                    try:
                        ...
                        if extracted_data:
                            ...
                            st.error("No valid marks data extracted from bundles.")
                    except Exception as e:
                        st.error(f"Error: {e}")
            ...
            if f_see and st.button("🚀 Process Bulk SEE"):
                ...
                if not (usn_col and cc_col and m_col):
                    ...
                    with st.spinner("Validating against registrations..."):
                        ...
                        if not records:
                            st.error("❌ Upload Failed. No valid records.")
                        else:
                            for i in range(0, len(records), 500):
                                supabase.table("student_results").upsert(records[i:i + 500]).execute()
                            st.success(f"✅ Successfully uploaded {len(records)} valid SEE records.")
                            if ignored_count > 0:
                                st.warning(f"⚠️ Blocked {ignored_count} unregistered records.")
        ...
        with col_s2:
            with st.form("manual_see"):
                ...
                if st.form_submit_button("Save SEE Mark"):
                    ...
                    if not regs:
                        st.error(f"❌ Student {s_usn} is NOT registered for {s_cc}.")
                    else:
            ...
            if mod_usn:
                try:
                    ...
                    if not fail_res.data:
                        st.success(f"🎉 Student {mod_usn} has no failing subjects!")
                    else:
                        actual_fails = [r for r in fail_res.data if r['grade'] not in ['PND', 'PENDING']]
                        if not actual_fails:
                            st.warning(f"Student {mod_usn} is PENDING in their subjects. Cannot apply grace marks yet.")
                        else:
                            ...
                            for r in actual_fails:
                                ...
                                c_cie = safe_float(r['cie_marks'], 0)
                                c_see = safe_float(r['see_raw'], 0)
                                c_tot = safe_float(r['total_marks'], 0)
                                c_grade = str(r['grade'])
                                
                                with st.expander(f"⚠️ {cc} - {title} (Current Grade: {c_grade})"):
                                    st.markdown(f"**Current Marks:** CIE: `{c_cie}` | SEE Raw: `{c_see}` | Scaled SEE: `{r.get('see_scaled', 'N/A')}` | Total: `{c_tot}`")
                                    with st.form(f"grace_form_{cc}"):
                                        ...
                                        if st.form_submit_button("✨ Apply Grace Marks & Recalculate"):
                                            if not grace_reason:
                                                ...
                                                scaled_see, tot, grd, gp, is_pass, healed_status = apply_grading_rules(
                                                    new_cie, new_see, r['exam_status'], cred, m_cie, m_see, conducted_for, is_pg
                                                )
                                                ...
                                                try:
                                                    ...
                                                    st.warning("Audit Log Warning: Log table might not exist yet.")
                                                ...
                                                update_data = {
                                                    ...
                                                    "exam_status": healed_status,
                                                    "is_graced": True,
                                                    "grace_marks_added": float(grace_marks)
                                                }
                                                supabase.table("student_results").upsert(update_data).execute()
                                                if is_pass:
                                                    st.success(f"✅ Grace marks applied and audited! Passed with Grade **{grd}**.")
                                                else:
                                                    st.warning(f"⚠️ Audited, but student still failing. New Grade: **{grd}**.")
                except Exception as e:
        ...
        if st.button("🖨️ Generate Master Ledgers & PDFs"):
            with st.spinner("Compiling institutional ledgers..."):
                try:
                    ...
                    # 🟢 FIX: Fetch master students and filter out DISCONTINUED students
                    raw_stu_res = fetch_all_records("master_students", "*")
                    stu_res = [s for s in raw_stu_res if s.get('status', 'ACTIVE') != 'DISCONTINUED']
                    ...
                    with zipfile.ZipFile(pdf_zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
                        for usn, courses in stu_courses.items():
                            ...
                            # If student was filtered out (discontinued), skip their entire loop
...
if show_reval:
    with t_rev:
        ...
        st.info("Upload or enter Revaluation marks. If the difference between the Original mark and Revaluation mark is > 15, the grade is frozen and flagged for Third Valuation. Otherwise, the higher mark is kept and the grade is instantly recalculated.")

        rev_tabs = st.tabs(["📂 Bulk Revaluation Upload", "📝 Manual Entry"])

        # --- HELPER: Fetch Grading Context ---
        def fetch_reval_context():
            db_res = fetch_all_records("student_results", filters={"cycle_id": selected_cycle_id})
            db_map = {(str(r['usn']).strip().upper(), str(r['course_code']).strip().upper()): r for r in db_res}

            # Get True Original Marks from Audit Log
            audit_res = fetch_all_records("marks_audit_log", "usn, course_code, old_see, created_at", filters={"cycle_id": selected_cycle_id})
            audit_df = pd.DataFrame(audit_res)
            true_original_map = {}
            if not audit_df.empty:
            ...
            return db_map, true_original_map, crs_map, branch_map, pg_branches
        ...
        with rev_tabs[0]:
            ...
            if f_rev and st.button("🚀 Execute Revaluation Rules", type="primary"):
                ...
                if not (usn_col and cc_col and m_col):
                    ...
                    with st.spinner("Analyzing original marks, checking > 15-mark differences, and applying rules..."):
                        try:
                            db_map, true_original_map, crs_map, branch_map, pg_branches = fetch_reval_context()

                            updates_list = []
                            audit_list = []
                            stats = {"upgraded": 0, "ignored": 0, "third_val": 0, "ignored_unregistered": 0}
                            ...
                            for _, r in df_rev.iterrows():
                                ...
                                if (u, c) in db_map and rv_mark is not None:
                                    ...
                                    # Compare against the true original evaluator mark (if exists), otherwise current mark
                                    true_orig_see = true_original_map.get((u, c), current_db_see)
                                    mark_diff = abs(rv_mark - true_orig_see)
                                    ...
                                    if mark_diff > 15:
                                        ...
                                        if rv_mark > true_orig_see:
                                            ...
                                            scaled_see, tot, grd, gp, is_pass, healed_status = apply_grading_rules(
                                                db_row['cie_marks'], rv_mark, db_row['exam_status'], cred, m_cie, m_see, conducted_for, is_pg
                                            )
                            ...
                            c1, c2, c3 = st.columns(3)
                            c1.metric("⬆️ Upgraded (RV > Orig)", stats["upgraded"])
                            c2.metric("➖ Ignored (Orig >= RV)", stats["ignored"])
                            c3.metric("🚨 3rd Valuations Triggered", stats["third_val"], delta="Diff > 15", delta_color="inverse")
                            
                            if stats["ignored_unregistered"] > 0:
                                st.warning(f"⚠️ {stats['ignored_unregistered']} rows were ignored because the USN/Course combination was not found.")
                        except Exception as e:
                            st.error(f"Processing Error: {e}")
        ...
        with rev_tabs[1]:
            ...
            st.write("Manually override a specific student's SEE mark for revaluation. The >15 rule applies automatically.")
            
            with st.form("manual_rv_form"):
                col_m1, col_m2, col_m3 = st.columns(3)
                rv_usn = col_m1.text_input("USN").strip().upper()
                rv_cc = col_m2.text_input("Course Code").strip().upper()
                rv_mark = col_m3.number_input("New RV Mark (SEE)", min_value=0.0, max_value=100.0, step=1.0)
                
                submit_rv = st.form_submit_button("💾 Process Revaluation Mark", type="primary", use_container_width=True)
                
                if submit_rv:
                    if not rv_usn or not rv_cc:
                        st.error("⚠️ Please enter both a USN and a Course Code.")
                    else:
                        with st.spinner(f"Evaluating RV rules for {rv_usn}..."):
                            try:
                                db_map, true_original_map, crs_map, branch_map, pg_branches = fetch_reval_context()
                                
                                if (rv_usn, rv_cc) not in db_map:
                                    st.error(f"❌ Cannot find an existing exam record for {rv_usn} in {rv_cc} for this cycle.")
                                else:
                                    db_row = db_map[(rv_usn, rv_cc)]
                                    current_db_see = safe_float(db_row.get('see_raw'), 0)
                                    old_grade = db_row.get('grade')
                                    
                                    true_orig_see = true_original_map.get((rv_usn, rv_cc), current_db_see)
                                    mark_diff = abs(rv_mark - true_orig_see)
                                    ...
                                    if mark_diff > 15:
                                        ...
                                        if rv_mark > true_orig_see:
                                            mc = crs_map.get(rv_cc, {})
                                            cred, m_cie, m_see, conducted_for = safe_float(mc.get('credits'), 4.0), safe_float(mc.get('max_cie'), 50.0), safe_float(mc.get('max_see'), 50.0), safe_float(mc.get('total_marks'), 100.0)
                                            is_pg = branch_map.get(rv_usn) in pg_branches
